...
import { services as fallbackServices } from '../data/companyData';
...
export default function Services() {
  ...
  useEffect(() => {
    ...
    getServices()
      .then((response) => {
        setServices(response.data.length ? response.data : fallbackServices);
        setLoading(false);
      })
      .catch(() => {
        ...
        <>
          {services.map((service, index) => (
            <article
              ...
              className={`ix-band${index % 2 === 1 ? ' ix-band--flip' : ''}${index % 2 === 1 ? ' ix-band--alt' : ''}`}
            >
              <div className="ix-band-visual">
                <span className="ix-band-num">{String(index + 1).padStart(2, '0')}</span>
                <img src={service.image_url || service.image} alt={service.title} loading="lazy" />
              </div>
          ...
          <section className="ix-cta-strip">
